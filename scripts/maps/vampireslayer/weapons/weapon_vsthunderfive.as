//based on HL shotgun
namespace vs_thunderfive
{

const int VSW_DEFAULT_GIVE				= 5;
const int VSW_MAX_CLIP 					= 5;
const int VSW_MAX_AMMO					= 19;

const int VSW_DAMAGE						= 22;
const int VSW_SHOTS						= 4; //88 damage total
const float VSW_TIME_DELAY				= 0.75;
const float VSW_TIME_DRAW				= 0.5;
const float VSW_TIME_IDLE				= 3.4;

const string VSW_ANIMEXT					= "onehanded";
const string MODEL_VIEW					= "models/vs/weapons/v_thunderfive.mdl";
const string MODEL_PLAYER				= "models/vs/weapons/p_thunderfive.mdl";
...
class weapon_vsthunderfive : CBaseVSWeapon
{
	...
	void Precache()
	{
		...
		g_Game.PrecacheGeneric( "sprites/vs/weapon_vsthunderfive.txt" );
	...
	bool GetItemInfo( ItemInfo& out info )
	{
		...
		info.iSlot				= vs::THUNDER5_SLOT-1;
		info.iPosition			= vs::THUNDER5_POSITION-1;
		info.iWeight			= vs::THUNDER5_WEIGHT;
	...
	bool AddToPlayer( CBasePlayer@ pPlayer )
	{
		...
		NetworkMessage m( MSG_ONE, NetworkMessages::WeapPickup, pPlayer.edict() );
			m.WriteLong( g_ItemRegistry.GetIdForName("weapon_vsthunderfive") );
		m.End();
	...
	void PrimaryAttack()
	{
		...
		//multiplayer spread
		self.FireBullets( VSW_SHOTS, vecSrc, vecAiming, vs::VECTOR_CONE_DM_SHOTGUN, 2048, BULLET_PLAYER_CUSTOMDAMAGE, 0, int(flDamage), m_pPlayer.pev );
		...
		if( self.m_flTimeWeaponIdle <  g_Engine.time )
		{
			...
			else if( m_iInSpecialReload != 0 )
			{
				...
				else
				{
					...
					g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_ITEM, arrsSounds[SND_EMPTY], VOL_NORM, ATTN_NORM, 0, 95 + Math.RandomLong(0, 0x1f) );
					m_iInSpecialReload = 0;
			...
			else
			{
				...
				self.m_flTimeWeaponIdle = g_Engine.time + (VSW_TIME_IDLE + Math.RandomFloat(3.0, (VSW_TIME_IDLE*4)));
			}
		}
	}
...
void Register()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "vs_thunderfive::weapon_vsthunderfive", "weapon_vsthunderfive" );
	g_ItemRegistry.RegisterWeapon( "weapon_vsthunderfive", "vs", "vsthunderfiveammo" );
}
